...
let cloud, writes = 0, offline = false;
...
window.syncTest = {
  get cloud() { return cloud; }, get writes() { return writes; },
...
try {
  ...
  await page.getByText("מסונכרן עם גוגל", { exact: true }).waitFor()
  ...
  await page.getByText("נשמר במכשיר · הסנכרון ממתין", { exact: true }).waitFor()
  await page.evaluate(() => {
    const remote = structuredClone(window.syncTest.cloud)
  ...
  console.log("PASS: automatic upload/download, quiet edits, offline conflicts, mobile resolution, sign-out cleanup (synthetic Firebase)")
} catch (error) {
