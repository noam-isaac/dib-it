...
test("semester-only group edits preserve the other semester's annual selection", () => {
  for (const available of [{}, catalogs]) {
    ...
    } }, importCatalogs(available))
    expect(result.plans[0].courses!["2026b"]).toEqual([course])
    expect(result.plans[0].pendingAnnualChanges).toBeUndefined()
    expect(reconcileActivePlan(roundTrip(result), importCatalogs(catalogs)).plans[0].courses!["2026b"]).toEqual([course])
  }
})
...
test("a loaded but incomplete catalog cannot discard removals or deselections", () => {
  for (const semester of ["2026a", "2026b"]) for (const remove of [false, true]) {
    ...
    const waiting = updateActivePlan(workspace, changed, importCatalogs(incomplete))
    expect(waiting.plans[0].pendingAnnualChanges).toHaveLength(1)
    expect(isScheduleBackup(waiting)).toBe(true)
    const completed = reconcileActivePlan(roundTrip(waiting), importCatalogs(catalogs))
...
test("offline removal and deselection survive reload, backups, and either catalog arrival order", () => {
  for (const remove of [false, true]) for (const first of ["2026a", "2026b"]) {
    ...
    workspace = updateActivePlan(workspace, edited, importCatalogs({}))
    ...
    workspace = reconcileActivePlan(workspace, importCatalogs({ [first]: catalogs[first] }))
    expect(workspace.plans[0].pendingAnnualChanges).toHaveLength(1)
    workspace = reconcileActivePlan(workspace, importCatalogs(catalogs))
    ...
    expect(reconcileActivePlan(workspace, importCatalogs(catalogs))).toEqual(workspace)
  }
})
...
test("the last explicit edit wins when both semesters are edited offline", () => {
  ...
  workspace = updateActivePlan(workspace, { ...activePlanView(workspace), courses: { "2026a": [], "2026b": [course] } })
  const result = reconcileActivePlan(roundTrip(workspace), importCatalogs(catalogs))
...
test("loading and no-op saves preserve conflicting selections and semester-specific metadata", () => {
  ...
  expect(reconcileActivePlan(before, importCatalogs(catalogs))).toEqual(before)
  expect(updateActivePlan(before, activePlanView(before), importCatalogs(catalogs))).toEqual(before)
  expect(updateActivePlan(before, { ...activePlanView(before), tab: "exams" }, importCatalogs(catalogs)).plans).toEqual(before.plans)
})
...
test("pending edits stay with their plan, survive duplication, and cannot affect a restored workspace", () => {
  ...
  expect(reconcileActivePlan(switched, importCatalogs(catalogs)).plans[0]).toEqual(edited.plans[0])
  const duplicated = addPlan(edited, "Copy", true)
  const reconciled = reconcileActivePlan(duplicated, importCatalogs(catalogs))
  expect(reconciled.plans[0]).toEqual(edited.plans[0])
  expect(reconciled.plans[1].courses).toEqual({ "2026a": [], "2026b": [] })
  expect(reconcileActivePlan(roundTrip(before), importCatalogs(catalogs))).toEqual(before)
})
...
test("annual deltas preserve untouched selections and metadata and replay idempotently", () => {
  ...
  for (const before of selections) for (const after of selections) for (const peer of selections) {
    ...
    const result = applyAnnualChanges(view, changes, importCatalogs({}))
    ...
    expect(applyAnnualChanges({ ...view, courses: result.courses }, changes, importCatalogs({}))).toEqual(result)
...
test("missing classification cannot acknowledge pending changes from another snapshot", () => {
  for (const change of [
    ...
    const result = applyAnnualChanges(view, [change], importCatalogs(catalogs))
...
test("a pending old-year edit does not block unrelated current-year reconciliation", () => {
  ...
  const result = reconcileActivePlan(workspace, importCatalogs(catalogs))
  expect(result.plans[0].courses?.["2026b"]).toEqual([course])
