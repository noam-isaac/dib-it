...
const App = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!semester) {
        const info = await cachedFetch(dataUrls.info, generalInfoSchema)
      ...
      const result = await cachedFetch(`${dataUrls.semesterCourses(semester)}?${startDateString}`, semesterCoursesSchema, assertCourseCatalog,
      )
      if (cancelled) return
      ...
      void cachedFetch(`${dataUrls.semesterCourses(otherSemester)}?${startDateString}`, semesterCoursesSchema, assertCourseCatalog,
