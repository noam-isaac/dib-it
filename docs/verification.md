...
- The dedicated Google Cloud project `dib-it-noam-isaac` was created with authorization. Firebase's MCP reported billing disabled; no billing account, trial or paid service was enabled. Adding Firebase still returns `403 PERMISSION_DENIED`, so live sign-in and backup remain unconfigured and unverified. [Firebase documents](https://firebase.google.com/docs/projects/use-firebase-with-existing-cloud-project) missing permissions or unaccepted Firebase Terms as possible causes; the account owner must resolve the console prompt before setup can continue. Do not enable billing to resolve setup or quota failures.
