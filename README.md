...
The active schedule name appears above the semester selector and in the registration export dialog. Click the name to switch schedules, or open the three-dot menu next to the semester and choose **מערכות שעות**. The dialog also creates, duplicates, renames, or deletes a schedule. Deletion requires confirmation, and the final schedule cannot be deleted. Each schedule keeps independent course/group selections, colors, and degree settings across all semesters. Theme, custom course catalogs, and exam practice history are shared.
...
A separate form is filled for each registering department, including its code and name. Department codes come from course-number prefixes, and names come from the course catalog’s faculty/department field. Course details, semester and form defaults are filled automatically. Missing or conflicting department names and other missing catalog text stay blank; complete or correct them in Word after downloading. The original form has 14 course rows; additional rows continue in another original form. Multiple forms download together as a ZIP. Leading zeroes are preserved. Unselected or stale groups are excluded. The year/semester digit boxes use the selected semester; the original printed heading is retained, so check that the supplied תשפ״ז form is appropriate for your registration. Student details remain in the dialog and downloaded files only; they are not stored in localStorage or Google backup.
